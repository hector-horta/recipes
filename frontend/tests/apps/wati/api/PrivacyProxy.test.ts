...
import { InputSanitizer, SecureAPI } from '@wati/src/api/PrivacyProxy';

vi.mock('@wati/src/db/db', () => ({
    db: {
        searchCache: {
...
vi.mock('@wati/src/utils/imageCache', () => ({
    cacheRecipeImages: vi.fn().mockResolvedValue(undefined)
}));

vi.mock('@wati/src/security/SecureVault', () => ({
    SecureVault: {
...
describe('SecureAPI', () => {
    ...
    it('should return cached recipes when cache hit', async () => {
        ...
        const { db } = await import('@wati/src/db/db');
