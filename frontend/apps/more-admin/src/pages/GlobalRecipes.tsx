...
export const GlobalRecipes: React.FC = () => {
  ...
  const bulkUpdateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['global-recipes'] });
      setSelectedIds([]);
      toast.success('Estado actualizado para las recetas seleccionadas');
    },
    onError: (error: any) => {
      toast.error(error?.message || 'Error al actualizar las recetas');
    }
  });
  ...
  const bulkDeleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['global-recipes'] });
      setSelectedIds([]);
      toast.success('Recetas eliminadas correctamente');
    },
    onError: (error: any) => {
      toast.error(error?.message || 'Error al eliminar las recetas');
    }
  });
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <motion.div variants={itemVariants}>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-4xl font-extrabold text-brand-forest tracking-tight">Recetas Globales</h2>
          </div>
          <p className="text-brand-text-muted font-medium">Catálogo base de Wati disponible para todos los inquilinos.</p>
        </motion.div>
        ...
        <div className="p-8 space-y-6">
          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="relative flex-1 w-full">
              ...
              <input
                type="text"
                placeholder="Buscar por nombre, ingredientes o slug..."
                value={searchTerm}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-brand-cream/20">
                  ...
                  <th className="px-8 py-5 text-xs font-bold text-brand-forest uppercase tracking-widest">Receta</th>
                  <th className="px-8 py-5 text-xs font-bold text-brand-forest uppercase tracking-widest">Dificultad</th>
                  <th className="px-8 py-5 text-xs font-bold text-brand-forest uppercase tracking-widest">Riesgo SIBO</th>
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-16 h-16 rounded-[1.25rem] bg-brand-sage/10 overflow-hidden flex items-center justify-center text-brand-sage shrink-0 border-2 border-brand-sage/20 group-hover:scale-105 transition-transform duration-300 shadow-sm">
                            {recipe.image_url ? (
                              <img src={recipe.image_url} alt={recipe.title_es} className="w-full h-full object-cover" />
                            ) : (
                              <ChefHat size={28} strokeWidth={1.5} />
                            )}
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span className="font-bold text-brand-forest text-lg leading-tight truncate">{recipe.title_es}</span>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold text-brand-forest capitalize">{recipe.sibo_risk_level}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <Badge 
                        ...
                        >
                          {recipe.status === 'published' ? '● Publicada' : recipe.status === 'draft' ? '○ Borrador' : 'Archivada'}
                        </Badge>
                      </td>
                    ...
                    <td colSpan={5} className="px-8 py-20 text-center">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="space-y-1">
                          <p className="text-brand-forest font-black text-xl">Sin coincidencias</p>
                          <p className="text-brand-text-muted font-medium">Prueba a ajustar tus filtros de búsqueda.</p>
                        </div>
          ...
          <div className="p-8">
                ...
                {recipesLoading ? (
                  ...
                  >
                    <div className="aspect-square rounded-[2rem] overflow-hidden mb-4 relative bg-brand-cream/40">
                      ...
                      {recipe.image_url ? (
                        <img src={recipe.image_url} alt={recipe.title_es} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                      ) : (
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-brand-forest/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-6">
                         <div className="flex gap-2">
                           ...
                           <button 
                             onClick={() => {
                               if (confirm('¿Eliminar receta?')) deleteMutation.mutate(recipe.id);
                             }}
                    ...
                    <div className="px-2 pb-2">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] font-bold text-brand-text-muted uppercase tracking-tighter">
                          Riesgo {recipe.sibo_risk_level}
                        </span>
                      </div>
                      <h3 className="font-bold text-brand-forest text-lg leading-tight mb-2 line-clamp-1">{recipe.title_es}</h3>
                      <div className="flex items-center justify-between text-[11px] font-bold text-brand-text-muted/60">
                        <div className="flex items-center gap-3">
                          <span className="flex items-center gap-1"><Clock size={12} /> {recipe.prep_time_minutes + recipe.cook_time_minutes}m</span>
                          <span className="flex items-center gap-1"><Users size={12} /> {recipe.servings}</span>
                        </div>
                        <span className="uppercase tracking-widest text-[9px] px-2 py-0.5 bg-brand-cream rounded-md">{recipe.difficulty}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
            ...
            {!recipesLoading && filteredRecipes?.length === 0 && (
              <div className="py-20 text-center">
                <p className="text-brand-text-muted">No se encontraron recetas.</p>
              </div>
            )}
          </div>
        )}
          ...
          >
            <div className="bg-brand-forest text-white rounded-[2rem] p-6 shadow-2xl flex items-center justify-between gap-6 backdrop-blur-xl bg-opacity-95 border border-white/10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-sm">Recetas Seleccionadas</p>
                  <button 
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  onClick={() => {
                    if (confirm(`¿Eliminar ${selectedIds.length} recetas?`)) {
      ...
      <Modal
        ...
        title={editingRecipe ? 'Perfeccionar Receta' : 'Nueva Receta Maestra'}
        className="max-w-6xl"
      >
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <form onSubmit={handleSubmit} id="recipe-form" className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <AnimatePresence mode="wait">
                      {activeLang === 'es' ? (
                        ...
                        >
                          ...
                          <Input
                            placeholder="Ej: Wild Mushroom Risotto"
                            className="h-12 rounded-xl border-2 border-brand-sage/10 focus:border-brand-sage/40 transition-all font-bold"
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black text-brand-forest uppercase tracking-widest flex items-center gap-2">
                        <span className="w-1.5 h-1.5 bg-brand-forest rounded-full" />
                        Identificador (Slug)
                      </label>
                      <Input
                        placeholder="risotto-setas-silvestres"
                        className="h-12 rounded-xl border-2 border-brand-sage/10 focus:border-brand-sage/40 transition-all font-mono text-xs lowercase"
                        value={formData.slug}
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-brand-forest uppercase tracking-widest">Dificultad</label>
                    <select 
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-brand-forest uppercase tracking-widest">Estado de Publicación</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-brand-forest uppercase tracking-widest flex items-center gap-1">
                      <AlertTriangle size={14} className="text-brand-sage" /> Riesgo SIBO
                    </label>
              ...
              <section className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-brand-forest font-black uppercase tracking-[0.2em] text-xs">
                    <div className="w-8 h-[2px] bg-brand-sage/30" />
                    Ingredientes ({formData.ingredients.length})
                  </div>
                  <motion.button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {formData.ingredients.length === 0 && (
                    <div className="col-span-2 py-10 border-2 border-dashed border-brand-sage/20 rounded-3xl flex flex-col items-center justify-center text-brand-text-muted gap-2">
                      ...
                      <p className="text-xs font-bold uppercase tracking-widest opacity-40">No hay ingredientes definidos</p>
                    </div>
                  )}
                </div>
              </section>
                ...
                <div className="space-y-4">
                    ...
                    {formData.steps.map((step, idx) => (
                      ...
                      >
                        ...
                        <textarea
                          ...
                          placeholder={`Describe el proceso para el paso ${idx + 1}...`}
                          value={step}
                          onChange={(e) => {
                            const newSteps = [...formData.steps];
                  ...
                  {formData.steps.length === 0 && (
                    <div className="py-10 border-2 border-dashed border-brand-sage/20 rounded-3xl flex flex-col items-center justify-center text-brand-text-muted gap-2">
                      <Info size={24} className="opacity-20" />
                      <p className="text-xs font-bold uppercase tracking-widest opacity-40">Define el procedimiento paso a paso</p>
                    </div>
                  )}
                </div>
              </section>
          ...
          <div className="lg:col-span-4 space-y-8">
            <section className="space-y-4">
              <label className="text-[11px] font-black text-brand-forest uppercase tracking-widest flex items-center gap-2">
                <ImageIcon size={16} className="text-brand-sage" /> Visual Studio (IA)
              </label>
              <div className="aspect-[4/5] rounded-[2.5rem] bg-brand-cream/50 border-4 border-white shadow-2xl overflow-hidden relative group">
                <AnimatePresence mode="wait">
                  {formData.image_url ? (
                    ...
                    >
                      ...
                      <p className="text-sm font-black uppercase tracking-widest leading-tight">Sin Imagen Magistral</p>
                      <p className="text-[10px] font-medium mt-2">La IA generará una imagen basada en tu descripción técnica al guardar.</p>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                {refreshImageMutation.isPending && (
                  <div className="absolute inset-0 bg-brand-forest/60 backdrop-blur-md flex flex-col items-center justify-center text-white p-8 text-center z-20">
                    ...
                    <p className="text-lg font-black tracking-tight leading-tight">Manifestando tu visión...</p>
                    <p className="text-xs font-medium opacity-70 mt-2 italic">Esto requiere un procesamiento neuronal intenso (20-30s)</p>
                  </div>
                )}
              </div>
              ...
              {editingRecipe && (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-xs font-black text-brand-forest uppercase tracking-widest">Visual Studio (AI)</h4>
                  </div>
                  <textarea
                    placeholder="Describe ajustes (ej: 'Iluminación dorada de atardecer', 'Añade un fondo rústico de madera')..."
                  ...
                  >
                    ...
                    <span>{refreshImageMutation.isPending ? 'Manifestando...' : 'Regenerar con IA'}</span>
                    {refreshImageMutation.isPending && (
                      <motion.div 
            ...
            <section className="space-y-4">
              <label className="text-[11px] font-black text-brand-forest uppercase tracking-widest flex items-center gap-2">
                <Filter size={16} className="text-brand-teal" /> Etiquetas & Alérgenos
              </label>
              <div className="flex flex-wrap gap-2 p-6 bg-brand-cream/30 rounded-[2rem] border border-brand-sage/10 max-h-[300px] overflow-y-auto custom-scrollbar">
                ...
                {tags?.length === 0 && <p className="text-xs text-brand-text-muted italic text-center w-full py-4 opacity-40">Carga etiquetas en el diccionario primero.</p>}
              </div>
            </section>
            ...
            <div className="p-6 bg-brand-sage/5 rounded-[2rem] border border-brand-sage/10">
              <h4 className="text-[10px] font-black text-brand-forest uppercase tracking-widest mb-3 flex items-center gap-2">
                <Info size={12} /> Guía de Estilo
              </h4>
              <ul className="text-[10px] text-brand-text-muted space-y-2 font-medium">
                <li className="flex gap-2">
                  <span className="text-brand-sage">•</span>
                  <span>El **Slug** es permanente tras la creación para mantener el SEO.</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-brand-sage">•</span>
                  <span>Describe los ingredientes con precisión para mejores resultados de IA.</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-brand-sage">•</span>
                  <span>El **Riesgo SIBO** es vital para la seguridad alimentaria del usuario.</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="mt-10 pt-8 border-t border-brand-sage/10 flex flex-col md:flex-row gap-4">
          ...
          >
            {createMutation.isPending || updateMutation.isPending ? (
              ...
              <>
                <span>{editingRecipe ? 'Consolidar Receta' : 'Publicar en el Catálogo'}</span>
                <ArrowRight size={18} />
              </>
            )}
          </Button>
